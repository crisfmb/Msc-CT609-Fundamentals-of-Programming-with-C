...
void load_data(const char *files[MAX_LOCATIONS], Location *const locArray, Rainfall *const rainArray, int *records, int *filepaths, int prefixLength)
{
    ...
    for (i = 0; i < maxFiles; i++)
    {
        ...
        { // check if opening the file was successful
            while (!feof(filePtr) && *records < MAX_RECORDS)
            {                                                              // while the end of the file is not reached
                ...
                if (i == 0 && (rainArray[*records].year == NULL || rainArray[*records].month == NULL) && !feof(filePtr))
                {                     // check if first record is NULL (meaning invalid values or empty file)
                    (*records) = 0;   // reset records count
                    (*filepaths) = 0; // reset filepaths count
                    maxLoc = 0;       // reset maxLoc
                    break;            // exit loop
                }
                else if (rainArray[*records].year != NULL && !feof(filePtr))
                {                 // check for blank lines and move to next line if found
...
void sort_raindays_by_year_month(const char *files[MAX_LOCATIONS], Location *const locArray, Rainfall *const rainArray, int records, int filepaths, int prefixLength)
{
    ...
    for (i = 0; i < records; i++)
    {
        ...
        { // check if the location number equals the requested location
    ...
    if (filePtr == NULL)
    {                                                 // check if opening file was successfull
...
void highest_volume_rain_one_day(Location *const locArray, Rainfall *const rainArray, int records, int filepaths)
{
    ...
    for (i = 0; i < copyItems; i++)
    {
        if (mostRainfall < copyRainArray[i + 1].mostRainfall)
        {                                                     // check if mostRainfall is less than the subsequent record
