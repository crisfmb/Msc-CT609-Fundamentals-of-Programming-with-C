/* Cristina Borges - Fundamentals of Programming Assignment 5 - Question 1
...
7. Save the sorted data in a new file to be used in Question 2 - per course facilitator this part can be skipped
8. Display your results and discuss which algorithm performed better on your data set.
 ...
 Insert Sort is the second alogorithm I've done/*/
...
#define SIZE 1440  // define SIZE constant for us in array length - array size was determined based on the number of data points downloaded from data set
...
int main()
{
    ...
    int sub = 0;         // declare and initialize counter for number of times array entry substitued in algorithm
    double minimum;      // declare variable for storing minimum value if you switch entries
    int position;        // delcare position variable for storing position when a switch happens
    ...
    int j;               // declare counter for number of substitutions made in sort

    startTime = clock(); // set start time

    // for loop that goes through the entire array

    for (j = 0; j < SIZE; j++) //for loop condition; count number of substitutions subject to max of 1440 and increments
    {
        minimum = array[j];            // set minimum value equal to first element of remaining unsorted array
        position = j;                  // set position equal to first index of remaining unsorted array
        for (i = j + 1; i < SIZE; i++) // for loop condition;
        {
            if (array[i] < minimum)
            {                       // compare minimum value to rest of array
        ...
        if (position != j)
        {
            array[position] = array[j]; // switch array elements
            array[j] = minimum;         // set j element of array to minimum value
            sub++;                      // increments switches by 1
        }
    }

    endTime = clock(); // stops timer so time taken can be calculated

    totalTime = ((double)(endTime - startTime)) / CLOCKS_PER_SEC; // calculates time taken per system clock and converts to seconds

    // print array results

    for (i = 0; i < SIZE; i++) //for loop condition, connts number of comparisons, checks if it's less than 1440 and increments by 1
    {
        if (i == 0) //if loop condtion when comparisons are nil
        {
            printf("{%1.4f, ", array[i]); // prints first array element as a float with 4 digits after decimal point
        }
        else if (i == SIZE - 1)
        {
            printf("%1.4f}\n", array[i]); // prints last array element as a float with 4 digits after decimal point
        }
        else
        {
            printf("%1.4f, ", array[i]); // prints all other array elements as float with 4 digits after decimal
        }
    }

    // print results

    printf("\nThe total number of swaps is: %d", sub);                 //gives total number of switches
    printf("\nThe total number of comparisons is: %d", comparisons);   //gives total number of comparisons
    printf("\nThe total time to execute is: %f seconds\n", totalTime); //gives time taken
